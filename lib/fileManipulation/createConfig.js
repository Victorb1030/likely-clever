...
var createFileResult = function buildConfigFile( credentials, appConfig ) {
    ...
    var configureScriptLocation = __dirname + String(appConfig.configureScriptLocation);
    //  /set input parameters
    
	var cmd;
	
	if( fs.existsSync(configureScriptLocation) ){
		
		cmd = 'node ' + configureScriptLocation + ' ' + username + ' ' + pass + ' ' + siteId;
	}else {
		return {error:"Cannot find configureScriptLocation " + configureScriptLocation};
	}
    console.log("CMD: " + cmd);
    
    var response;

    var cwdir = __dirname + appConfig.scriptWorkingDirectory;
